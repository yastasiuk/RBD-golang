...
func (s *Store) CreateCollection(name string, cfg *CollectionConfig) (bool, *Collection) {
	slog.Debug("CreateCollection", "name", name, cfg)
...
func (s *Store) Dump() ([]byte, error) {
	...
	if err != nil {
		slog.Error("Failed to Dump() store:", err)
		return nil, err
	}
...
func NewStoreFromFile(filename string) (*Store, error) {
	...
	if err != nil {
		slog.Error("Failed to read file:", "filename", filename, err)
		return nil, err
	}

	store, err := NewStoreFromDump(data)
	if err != nil {
		slog.Error("Failed on store from dump creation:", err)
		return nil, err
	}
...
func (s *Store) DumpToFile(filename string) error {
	...
	if err != nil {
		slog.Error("Error on WriteFile()", err)
		return err
	}
